...
object Engine:
  /** The Engine trait defines the core logic of the game, responsible for applying actions to the
    * game state. It takes the current game state and a game action, and returns the new game state
    */
  trait Engine:
  ...
  object ImmediatelyEndEngine extends Engine:
    override def applyAction(state: GameState, action: GameAction): GameState =
