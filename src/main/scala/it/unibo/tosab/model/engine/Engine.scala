...
/** Result of an engine step.
  *
  * @param nextState
  *   state produced after applying an action and post-rules
  * @param events
  *   domain events emitted by the transition
  */
...
trait Engine:
  /** Starts a new round by recomputing the turn queue from alive units.
    ...
    *   current game state
    * @return
    *   state with refreshed queue, or `GameOver` when no unit can act
    */
  ...
  /** Applies one command intent to the current state.
    ...
    *   actor/action intent
    * @return
    *   resulting state plus emitted domain events
    */
